#!/usr/bin/env python3
"""Verify demo-spring OrderService#payOrder context includes expected neighbors."""
...
def main() -> int:
    ...
    target = resolve_stable_id(DB_PATH, display_name="payOrder", prefer_type=False)
    required = [
        line.strip()
    ...
    if "payOrder" not in result.text:
        print("missing target method signature: payOrder", file=sys.stderr)
        return 1
    if missing:
        print("missing types in context:", ", ".join(missing), file=sys.stderr)
        return 1

    print(f"OK — payOrder + all {len(required)} expected types present")
    return 0
