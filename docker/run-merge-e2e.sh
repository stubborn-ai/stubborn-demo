        ...
        (
            ...
            if [[ -f "${DB_PATH}" ]]; then
                stubborn index --scip index.scip --out "${DB_PATH}" \
                    --merge --paths "${PROBE_RELATIVE_PATH}" >/dev/null
            fi
        ) || true
    fi
}
...
echo "== orders-demo merge E2E (Docker) =="
...
stubborn index --scip index.scip --out "${DB_PATH}" \
    --merge --paths "${PROBE_RELATIVE_PATH}"
stubborn info "${DB_PATH}"
...
if [[ "$(json_field "${after}" run_id)" != "${baseline_run_id}" ]]; then
    echo "Merge changed index_run_id from ${baseline_run_id} to $(json_field "${after}" run_id)" >&2
    exit 1
fi
...
if [[ "$(json_field "${after}" merge_count)" -lt 1 ]]; then
    echo "Expected merge_count >= 1 after merge" >&2
    exit 1
fi
if [[ "$(json_field "${after}" relative_path)" != "${PROBE_RELATIVE_PATH}" ]]; then
    echo "Probe relative_path mismatch: $(json_field "${after}" relative_path)" >&2
    exit 1
fi
...
if [[ "$(json_field "${final}" merge_count)" -lt 2 ]]; then
    echo "Expected merge_count >= 2 after add/remove cycle" >&2
    exit 1
fi
CLEANUP_COMPLETE=1
